...
/**
 * irdma_vfdev_from_fpm - return vf dev ptr for hmc function id
 * @dev: pointer to the device structure
 * @hmc_fn_id: hmc's function id
 */
struct irdma_vfdev *irdma_vfdev_from_fpm(struct irdma_sc_dev *dev, u16 hmc_fn_id)
{
	...
	for (idx = 0; idx < dev->num_vfs; idx++) {
		if (dev->vf_dev[idx] &&
		    (dev->vf_dev[idx]->pmf_index == hmc_fn_id)) {
			    ...
			    struct irdma_hmc_create_obj_info *info)
{
	...
	if (!dev->privileged)
		return irdma_vchnl_vf_add_hmc_objs(dev, info->rsrc_type,
						   info->start_idx,
						   info->count);
			 ...
			 struct irdma_hmc_del_obj_info *info, bool reset)
{
	...
	if (!dev->privileged) {
		if (!reset) {
			ret_code = irdma_vchnl_vf_del_hmc_obj(dev, info->rsrc_type,
							      info->start_idx,
							      info->count);
		}
