			...
			struct irdma_add_page_info *info)
{
	...
	struct irdma_hmc_info *hmc_info = info->hmc_info;
	struct irdma_chunk *chunk = info->chunk;
	struct irdma_manage_vf_pble_info vf_pble_info;
	int status = 0;
	...
	if (!dev->privileged) {
		status = irdma_vchnl_vf_add_hmc_objs(
	...
	if (!dev->privileged) {
		vf_pble_info.first_pd_index = (u16)info->idx.rel_pd_idx;
		...
		status = irdma_hw_manage_vf_pble_bp(dev, &vf_pble_info);
		if (status)
			goto error;
	}
	chunk->fpm_addr = pble_rsrc->next_fpm_addr;
