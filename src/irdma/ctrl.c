		       ...
		       struct irdma_vsi_init_info *info)
{
	...
	if (vsi->register_qset || (!vsi->dev->privileged &&
				   vsi->dev->hw_attrs.uk_attrs.hw_rev >= IRDMA_GEN_2)) {
				   ...
				   enum irdma_ws_node_op node_op, u64 scratch)
{
	...
	set_64bit_val(wqe, 32,
		      ...
		      FIELD_PREP(IRDMA_CQPSQ_WS_FAILING_PORT, info->failing_port) |
		      FIELD_PREP(IRDMA_CQPSQ_WS_FAILOVER_PORT, info->failover_port) |
...
int irdma_sc_init_iw_hmc(struct irdma_sc_dev *dev, u16 hmc_fn_id)
{
	...
	if (hmc_fn_id == dev->hmc_fn_id) {
		...
		vf_dev = irdma_vfdev_from_fpm(dev, hmc_fn_id);
		if (!vf_dev)
			return -EINVAL;

		hmc_info = &vf_dev->hmc_info;
		hmc_fpm_misc = &vf_dev->hmc_fpm_misc;
		iw_vf_idx = vf_dev->iw_vf_idx;
		ibdev_dbg(to_ibdev(dev),
			  "HMC: vf_dev %p, hmc_info %p, hmc_obj %p\n", vf_dev,
			  hmc_info, hmc_info->hmc_obj);
		if (!vf_dev->fpm_query_buf) {
			if (!dev->vf_fpm_query_buf[iw_vf_idx].va) {
				ret_code = irdma_alloc_query_fpm_buf(dev,
			      ...
			      struct cqp_cmds_info *pcmdinfo)
{
	...
	case IRDMA_OP_RESUME:
		status = irdma_sc_resume_qp(pcmdinfo->in.u.suspend_resume.cqp,
					    ...
					    pcmdinfo->in.u.suspend_resume.scratch);
		break;
	case IRDMA_OP_MANAGE_VF_PBLE_BP:
		status = irdma_manage_vf_pble_bp(pcmdinfo->in.u.manage_vf_pble_bp.cqp,
	...
	case IRDMA_OP_WS_ADD_NODE:
		status = irdma_sc_manage_ws_node(pcmdinfo->in.u.ws_node.cqp,
						 ...
						 pcmdinfo->in.u.ws_node.scratch);
		break;
	case IRDMA_OP_WS_FAILOVER_START:
		status = irdma_sc_manage_ws_node(pcmdinfo->in.u.ws_node.cqp,
						 &pcmdinfo->in.u.ws_node.info,
						 IRDMA_FAILOVER_START,
						 pcmdinfo->in.u.ws_node.scratch);
		break;
	case IRDMA_OP_WS_FAILOVER_COMPLETE:
		status = irdma_sc_manage_ws_node(pcmdinfo->in.u.ws_node.cqp,
						 &pcmdinfo->in.u.ws_node.info,
...
int irdma_sc_dev_init(struct irdma_sc_dev *dev, struct irdma_device_init_info *info)
{
	...
	if (!dev->privileged) {
		ret_code = irdma_vchnl_vf_get_hmc_fcn(dev);
		if (ret_code) {
			ibdev_dbg(to_ibdev(dev),
				  "DEV: Get HMC function rc = 0x%0x\n",
				  ret_code);
