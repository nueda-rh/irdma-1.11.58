			 ...
			 void *ptr)
{
	...
	case NETDEV_CHANGEADDR:
		irdma_add_arp(iwdev->rf, local_ipaddr, netdev->dev_addr);
			  ...
			  void *ptr)
{
	...
	case NETDEV_UP:
	case NETDEV_CHANGEADDR:
		irdma_add_arp(iwdev->rf, local_ipaddr6, netdev->dev_addr);
		    ...
		    void *ptr)
{
	...
	case NETEVENT_NEIGH_UPDATE:
		...
		if (neigh->nud_state & NUD_VALID)
			irdma_add_arp(iwdev->rf, local_ipaddr, neigh->ha);
		else
 ...
 * @pmf_idx: pmf or hmc function index for the vf_dev
 */
			      ...
			      struct irdma_dma_mem *mem)
{
	...
	status = irdma_obj_aligned_mem(dev_to_rf(dev), mem,
			      ...
			      struct irdma_vfdev *vf_dev)
{
	...
	if (cdev_info->ops->get_vf_info(cdev_info, vf_dev->vf_id, &port_info))
		return;

	vf_dev->port_vlan_en = port_info.port_vlan_id ? true : false;
}
					  ...
					  bool enable)
{
	...
	if (!vf_vsi)
		return NULL;
	if (!enable) {
		cdev_info->ops->update_vport_filter(cdev_info, vf_vsi->vsi_idx, false);
		kfree(vf_vsi);
		vf_vsi = NULL;
		vf_dev->vf_vsi = NULL;
	} else {
		cdev_info->ops->update_vport_filter(cdev_info, vf_vsi->vsi_idx, true);
	}
...
int irdma_upload_qp_context(struct irdma_qp *iwqp, bool freeze, bool raw)
{
	...
	if (ret)
		goto error;
	ibdev_dbg(to_ibdev(dev), "QP: PRINT CONTXT QP [%d]\n", info->qp_id);
	{
		u32 i, j;
