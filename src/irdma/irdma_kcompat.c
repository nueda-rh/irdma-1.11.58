...
static inline u8 irdma_get_vlan_ndev_prio(struct net_device *ndev, u8 prio)
{
				    ...
				    u8 *dmac)
{
	...
	if (ah_info->vlan_tag < VLAN_N_VID) {
...
#else
		struct net_device *ndev = sgid_attr->ndev;
		...
		ah_info->vlan_tag |=
			(u16)irdma_get_vlan_ndev_prio(ndev, rt_tos2priority(ah_info->tc_tos))
					<< VLAN_PRIO_SHIFT;
#ifdef SET_ROCE_CM_INFO_VER_3
		rcu_read_unlock();
#endif
					       ...
					       struct ib_udata *udata)
{
	...
	if (err) {
		ibdev_dbg(&iwdev->ibdev, "CQP create AH timed out");
		goto err_gid_l2;
	}
			      ...
			      u16 *vlan_id)
{
	...
	rcu_read_lock();
	ndev = rcu_dereference(sgid_attr->ndev);
	iwqp->ctx_info.user_pri = irdma_get_vlan_ndev_prio(ndev,
			      ...
			      u16 *vlan_id)
{
	...
	av->net_type = kc_rdma_gid_attr_network_type(sgid_attr,
						     ...
						     &sgid);
	rdma_gid2ip((struct sockaddr *)&av->sgid_addr, &sgid_attr->gid);
	iwqp->ctx_info.user_pri = irdma_get_vlan_ndev_prio(sgid_attr->ndev,
							   iwqp->ctx_info.user_pri);
	iwqp->sc_qp.user_pri = iwqp->ctx_info.user_pri;
...
{
	const char *name = iwdev->lag_active ? "irdma_bond%d" : "irdma%d";
	int i;
