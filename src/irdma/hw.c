	...
	do {
		...
		case IRDMA_AE_CQ_OPERATION_ERROR:
			ibdev_err(&iwdev->ibdev,
				  "Processing CQ[0x%x] op error,an iWARP related AE for CQ misc = 0x%04X\n",
			...
			if (!iwcq) {
				spin_unlock_irqrestore(&rf->cqtable_lock,
						       flags);
				ibdev_dbg(&iwdev->ibdev, "cq_id %d is already freed\n",
					  info->qp_cq_id);
				continue;
			}
		...
		default:
			ibdev_err(&iwdev->ibdev, "abnormal ae_id = 0x%x bool qp=%d qp_id = %d  ae_source=%d\n",
 ...
 */
static void irdma_dpc(unsigned long t)
{
 ...
 * @t: tasklet_struct ptr
 */
static void irdma_ceq_dpc(unsigned long t)
{
...
static void irdma_destroy_aeq(struct irdma_pci_f *rf)
{
	...
	if (!rf->msix_shared) {
		/* Non-privileged drivers call the IDC..*/
		if (rf->sc_dev.privileged)
			rf->sc_dev.irq_ops->irdma_cfg_aeq(&rf->sc_dev, rf->iw_msixtbl->idx, false);
		else
			rf->irdma_cfg_aeq(&rf->sc_dev, rf->iw_msixtbl->idx, false);
...
static void irdma_del_ceq_0(struct irdma_pci_f *rf)
{
	...
	if (rf->msix_shared) {
		...
		if (rf->sc_dev.privileged)
			...
			rf->irdma_cfg_ceq(&rf->sc_dev, msix_vec->ceq_id,
					  msix_vec->idx, false);
				...
				u32 ceq_id, struct irdma_msix_vector *msix_vec)
{
	...
	if (rf->sc_dev.privileged)
		...
		rf->irdma_cfg_ceq(&rf->sc_dev, ceq_id, msix_vec->idx, true);
...
static int irdma_cfg_aeq_vector(struct irdma_pci_f *rf)
{
	...
	if (rf->sc_dev.privileged)
		...
		rf->irdma_cfg_aeq(&rf->sc_dev, msix_vec->idx, true);
...
static int irdma_initialize_dev(struct irdma_pci_f *rf)
{
	...
	if (!rf->sc_dev.privileged)
		status = irdma_vchnl_vf_get_vlan_parsing_cfg(&rf->sc_dev,
							     &rf->vlan_parse_en);
	else
		rf->vlan_parse_en = 1;
	if (status)
...
void irdma_ctrl_deinit_hw(struct irdma_pci_f *rf)
{
	...
	case CQP_CREATED:
		...
		if (!rf->sc_dev.privileged)
			irdma_vchnl_vf_put_hmc_fcn(&rf->sc_dev);
		fallthrough;
	case INITIAL_STATE:
...
/**
 * irdma_hw_manage_vf_pble_bp - manage vf pbles
 * @dev: hardware control device structure
 * @info: info for managing pble
 */
int irdma_hw_manage_vf_pble_bp(struct irdma_sc_dev *dev,
			       struct irdma_manage_vf_pble_info *info)
{
	struct irdma_pci_f *rf = dev_to_rf(dev);
	struct irdma_manage_vf_pble_info *hw_info;
	struct irdma_cqp_request *cqp_request;
	struct cqp_cmds_info *cqp_info;
	int status;
	bool wait = true;

	if (rf->init_state < CCQ_CREATED)
		wait = false;
	...
	cqp_info->cqp_cmd = IRDMA_OP_MANAGE_VF_PBLE_BP;
	cqp_info->post_sq = 1;
	cqp_info->in.u.manage_vf_pble_bp.cqp = &rf->cqp.sc_cqp;
