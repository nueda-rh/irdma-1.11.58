			    ...
			    struct irdma_ws_node *node, u8 cmd)
{
	...
	node_info.failover_port = node->failover_port;
...
static void irdma_remove_leaf(struct irdma_sc_vsi *vsi, u8 user_pri)
{
	...
	if (!vsi->dev->privileged) {
		...
		ret = irdma_vchnl_vf_manage_ws_node(vsi->dev, false, user_pri, NULL);
		if (ret)
			ibdev_dbg(to_ibdev(vsi->dev),
				  "VIRT: Send message failed 0x%0x\n", ret);
...
int irdma_ws_add(struct irdma_sc_vsi *vsi, u8 user_pri)
{
	...
	if (!vsi->dev->privileged) {
		...
		ret = irdma_vchnl_vf_manage_ws_node(vsi->dev, true, user_pri,
						    &vf_qs_handle);
		if (ret) {
			ibdev_dbg(to_ibdev(vsi->dev),
				  "VIRT: Send message failed 0x%0x\n", ret);
		...
		vsi->qos[user_pri].qs_handle = vf_qs_handle;
 ...
 * @failover_port: Port number to become active
 */
void irdma_ws_failover_cmd(struct irdma_sc_vsi *vsi, u8 cmd, u8 failing_port, u8 failover_port)
{
	...
	ws_node.failover_port = failover_port;
	irdma_ws_cqp_cmd(vsi, &ws_node, cmd);
	mutex_unlock(&vsi->dev->ws_mutex);
}
