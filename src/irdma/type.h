...
struct irdma_vchnl_if {
	int (*vchnl_recv)(struct irdma_sc_dev *dev, u16 vf_id, u8 *msg, u16 len);
	int (*vchnl_send)(struct irdma_sc_dev *dev, u16 vf_id, u8 *msg, u16 len);
	int (*vchnl_send_sync)(struct irdma_sc_dev *dev, u8 *msg, u16 len,
			       u8 *recv_msg, u16 *recv_len);
};

#define IRDMA_VCHNL_MAX_VF_MSG_SIZE 512
...
struct irdma_vfdev {
	...
	u8 *hmc_info_mem;
	u8 vf_msg_buf[IRDMA_VCHNL_MAX_VF_MSG_SIZE];
...
struct irdma_sc_dev {
	...
	struct irdma_hmc_info *hmc_info;
	struct irdma_virtchnl_rdma_caps vchnl_caps;
	struct irdma_vfdev *vf_dev[IRDMA_MAX_PE_ENA_VF_COUNT];
	u8 vf_recv_buf[IRDMA_VCHNL_MAX_VF_MSG_SIZE];
	u16 vf_recv_len;
